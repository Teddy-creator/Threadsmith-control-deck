...
- **Skill Orchestrator foundation**: v0.3.x 可以发现本地 Codex skills、解释 route health，并在缺少外部 skill 时回退到内置 mini protocols。
...
v0.3.x 内置 `brief`、`plan`、`debug`、`review`、`verify`、`closeout`、`handoff`、`recover`、`research` 等小协议。它们是 fallback kernel，不依赖用户本机一定安装了外部 skills。
...
### Phase History：项目路径账本

`.threadsmith/history/phases.jsonl` 记录项目已经走过哪些 phase、每个 phase 交付了什么、怎么验证、下一步接到哪里。它不是完整聊天记录，也不是每个 commit 的审计日志，而是帮助你跨线程、跨天恢复项目路径的 committed truth。

查看最近 phase：

```bash
npm run threadsmith:phase-history:summary -- "/path/to/your-project" --limit 8
```

更完整的边界和操作说明见 [Phase History Operator Guide](docs/guides/phase-history-operator-guide.md)。
