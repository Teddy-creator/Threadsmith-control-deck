...
- evidence 是证明层。
- docs reports / report artifacts 是人工审查层。
...
- `docs reports`
  这些文件是人为整理过的计划、审查、closeout 或验证报告。它们可以解释
  决策，但不能自动覆盖 committed truth。
- `audit`
  这些事件解释状态是怎么走到现在的。
- `runtime artifact`
...
2. committed truth
3. evidence
4. docs reports / report artifacts
5. derived packet
6. audit / runtime artifacts
7. chat memory
...
## Docs Reports and Report Artifacts
...
- 为什么做某个阶段；
- 当时观察到了什么；
- 某个候选 backfill、review、closeout 的人工判断是什么；
- 操作者后续应该怎样使用某个能力。

它们可以作为 evidence refs 被 committed truth 引用，但它们本身不是当前
truth。比如 `docs/reports/artifacts/recommended-*.json` 可以证明某次候选
生成或 backfill 的内容，但不会自动意味着当前 `.threadsmith/history/phases.jsonl`
已经采用了这些候选。

规则：

- `docs/plans/*` 可以描述将来要做什么，但不能证明已经完成。
- `docs/reports/*` 可以证明一次检查或 closeout 发生过，但不能替代
  `.threadsmith/acceptance-state.json`。
- `docs/reports/artifacts/*` 可以保存可复查的机器输出，但必须由人工报告或
  committed truth 明确引用后才进入决策链。
...
注意：`.threadsmith/history/phases.jsonl` 不是普通 audit trail。它是已采纳
phase boundary 的项目路径账本，属于 committed truth。`events.ndjson` 解释
发生过什么；phase history 记录哪些阶段被正式收进项目路径。
...
如果你需要查看项目之前完成过哪些 phase，或判断当前 phase 是否接得上历史，
看 [Phase History Operator Guide](../guides/phase-history-operator-guide.md)。
...
Docs report artifacts 是例外中的例外：如果它们是为了让 PR review 或后续
operator 复查某个候选结果而有意保存，可以提交；如果只是本地临时命令输出，
优先写到 `/tmp` 或 ignored runtime path。
